...
dev_ = ua_ -  project(as_vector([Constant(assemble(ua_[0]*dx)),Constant(assemble(ua_[1]*dx))]), V_1)

p_.rename("Ocean_pressure")
To_.rename("Ocean_temperature")
Ta_.rename("Atm_temperature")

outfile = File("CC_1_wo_Coriolis.pvd")
outfile.write(project(i_ua,V_1_out, name= "atm_velocity"),Ta_,project(i_uo,V_1_out, name= "ocean_velocity"),p_,To_,project(dev_,V_1_out, name= "atm_vel_dev"))
...
with open('CC_1_wo_Coriolis_INFO.txt','w') as file:
...
with open('CC_1_wo_Coriolis_INFO.txt','a') as file:
...
while (round(t,4)<=end):
    ...
    if iter%freq==0:
        ...
        dev = ua -  project(as_vector([Constant(assemble(ua[0]*dx)),Constant(assemble(ua[1]*dx))]), V_1)

        p.rename("Ocean_pressure")
        To.rename("Ocean_temperature")
        Ta.rename("Atm_temperature")
        outfile.write(project(ua,V_1_out, name= "atm_velocity"),Ta,project(uo,V_1_out, name= "ocean_velocity"),p,To,project(dev,V_1_out, name= "atm_vel_dev"))
