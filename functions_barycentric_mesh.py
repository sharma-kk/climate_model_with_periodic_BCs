...
# 1. take a mesh that you want to convert into barycentric mesh. Name it "baseMesh". It can be a standard mesh (from Firedrake lib) or a Gmsh mesh.
# 2. Pass the "baseMesh" to the first arg of BaryMeshHierarchy() function. The second arg indicates the level of refinement needed.
# 3. BaryMeshHierarchy(0 creates a hierarchy of meshes i.e. a set of meshes from which one can choose.
# For ex. passing 2 to the second arg means that the basemesh is refined twice and for each refinement there is a corresponding barycentric mesh (total 3 levels (0,1,2))
# 4. Select the desired mesh from created set of meshes.
# Below we can see one example of using these functions.
###############################

# baseMesh = Mesh("square.msh") # using the mesh created on Gmsh
# baseMesh = SquareMesh(3,3,1) # using the mesh from library

# msh = BaryMeshHierarchy(baseMesh, 2)
# print("LENGTH OF ARRAY:", len(msh))

# fmsh = msh[-1] # choosing the latest refined mesh (finest).

# mesh = fmsh

# V_1 = FunctionSpace(mesh, "CG", 1)
######################################################################
def ToroidalBaryMeshHierarchy(nx,ny,refinement_levels):
    """
    Generate a hierarchy of toroidal barycentric meshes

    :arg nx: The number of cells in the major direction (min 3)
    :arg ny: The number of cells in the minor direction (min 3)
    :arg refinement_levels: The number of refinement levels
    """
    baseMeshTorus = TorusMesh(nx,ny,1.0,0.5)
    ...
    bmh = BaryMeshHierarchy(baseMeshTorus,refinement_levels,callbacks=(before, after))
    return bmh

def ToroidalMeshHierarchy(nx,ny,refinement_levels):
    """
    Generate a hierarchy of toroidal  meshes
    ...
    def after(dm, i):
        ...
        try:
            ...
            warning("Vertex rebalancing from initial guess failed on level %i" % i)
    mh = MeshHierarchy(baseMeshTorus,refinement_levels,callbacks=(before, after))
    return mh


def Periodise(m):
    """Convert a torus (R= 1.0, r=0.5) into a square, thus returning a periodic Unit square mesh

     :arg m: Torus mesh
     """
    coord_fs = VectorFunctionSpace(m, FiniteElement('DG','triangle', 1, variant="equispaced"), dim=2)
...
def PeriodicUnitSquareBaryMeshHierarchy(N,refinement_levels):
    """
    Creates a hierarchy of periodic Unit square meshes with barycentric grids.

    :arg nx: The number of cells in the x-direction (min 3)
    :arg ny: The number of cells in the y-direction (min 3)
    :arg refinement_levels: The number of refinement levels in the mesh hierarchy
    """
    mh = ToroidalBaryMeshHierarchy(N,N,refinement_levels)
    meshes = tuple(Periodise(m) for m in mh)
    return meshes

def PeriodicUnitSquareMeshHierarchy(N,refinement_levels):
    """
    Creates a hierarchy of periodic Unit square meshes with normal grids.

    :arg nx: The number of cells in the x-direction (min 3)
    :arg ny: The number of cells in the y-direction (min 3)
    :arg refinement_levels: The number of refinement levels in the mesh hierarchy
    """
    mh = ToroidalMeshHierarchy(N,N,refinement_levels)
    meshes = tuple(Periodise(m) for m in mh)
    return meshes

# # The meshes look barycentric when number of cells are quite big.
# N = 60
#
# # mh_normal = PeriodicUnitSquareMeshHierarchy(N,0)
# mh_bary = PeriodicUnitSquareBaryMeshHierarchy(N,0)
#
# M_bary = mh_bary[-1]
# # M_normal = mh_normal[-1]
#
# V_1 = FunctionSpace(M_bary, "CG", 1)
# # V_2 = FunctionSpace(M_normal,"CG", 1)
# x,y = SpatialCoordinate(M_bary)
# # x,y = SpatialCoordinate(M_normal)
#
# # i_temp_normal = Function(V_2).interpolate((sin(pi*x)*sin(pi*y))**2)
# i_temp_bary = Function(V_1).interpolate((sin(pi*x)*sin(pi*y))**2)
# outfile = File("mesh_comparison.pvd")
# i_temp_bary.rename("temp_bary")
# # i_temp_normal.rename("temp_normal")
# outfile.write(i_temp_bary)
# # outfile.write(i_temp_normal)
