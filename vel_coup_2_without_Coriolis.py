# Climate model
# I removed the Coriolis force terms
import time
...
nu_a = 10**5 # kinematic viscosity of air at 25 degrees celcius is roughly equal to 10^-5
D_a = 10**5 # diffusion coefficient of air is roughly equal to 10^-5
...
nu_o = 10**4 # kinematic viscosity of ocean surface water at 25 degrees celcius is 10^-6
D_o = 10**3 # diffusion coefficient of ocean surface water is roughly equal to 10^-7
...
i_ua = project(as_vector([sin(2*pi*x), 0]), V_1)
...
outfile = File("VC_2_wo_Coriolis.pvd")
...
#---------------------------------#
lines = ["CLIMATE MODEL checking the incompressiblity of ocean velocity (Equations are non-dimensionalized with respect to atmosphere characteristic values) \n",
        ...
        "Initial ocean temperature: Constant(3000) \n",
        "Initial atmospheric velocity: [sin(2*pi*y), 0] \n",
        "Initial atmospheric temperature: Constant(3000) \n",
        "Printing results at every "+str(t_step)+" time steps (we call it t_step)"+ "\n"]
with open('VC_2_wo_Coriolis_INFO.txt','w') as file:
...
with open('VC_2_wo_Coriolis_INFO.txt','a') as file:
